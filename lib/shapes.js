...
// function to compile the SVG the user chooses
function logoShape(shape, shapeColor, text, textColor) {
    if (shape == 'Circle') {
        ...
        return `<svg width="300" height="200" version="1.1" xmlns="http://www.w3.org/2000/svg">
         <polygon points="${tri.XCoord[0]} ${tri.XCoord[1]}, ${tri.YCoord[0]} ${tri.YCoord[1]}, ${tri.ZCoord[0]} ${tri.ZCoord[1]}" fill="${tri.shapeColor}"/>
         <text x="50" y="60" text-anchor="middle" fill="${tri.textColor}">${tri.text}</text>
       </svg>`
    }
}

// function that takes the users info and creates the shape they chooses
function generateSVG(data) {
    return `${logoShape(data.shape, data.shapeColor, data.text, data.textColor)}`
}


module.exports = { Circle, Square, Triangle, logoShape, generateSVG }